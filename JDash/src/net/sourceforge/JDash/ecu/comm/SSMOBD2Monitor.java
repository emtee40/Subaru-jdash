...
public class SSMOBD2Monitor extends RS232Monitor
{
	...
	public List<Parameter> init(ParameterRegistry reg, InitListener initListener) throws Exception
	{
		...
		/*
		 * The rxPacket header should container the init packet, plus the last 3 bytes should be the
		 * respons header of 0x80 0xf0 0x10
		 */
		...
		for (ECUCap cap : getEcuCapabilities())
		{
			...
			{
				ECUParameter newParam = new ECUParameter((byte[]) cap.address_, cap.name_,
						cap.description_, 1);
		...
		try
		{
			...
			while (doRun_.booleanValue())
			{
				...
				for (ECUParameter param : this.params_)
				{
					if (param.isEnabled())
					{
						if (param.getLastFetchTime() + param.getPreferedRate() < System
								.currentTimeMillis())
						{
				...
				try
				{
					 ...
					 * into multiple packets. Since the max data size of an RS232 packet appears to
	...
	/***********************************************************************************************
	 * This method will perform the heavylifting hard work required to send and receive a packet
	 * with a given format. Packets sent and received with this method MUST conform to a particular
	 * format. RX Packets can start with any number of header bytes. But the next byte MUST be a
	 * "numbytes" value. This is the number of data bytes to expect on the input stream. After that
	 * number of bytes are received, the last byte will be a checksum byte for the entire returned
	 * packet.
	 * 
	 * 
	 * @param txPacket
	 *            IN - the preformatted tx packet to be sent out the OutputStream os.
	 * @param rxDataLenIndex
	 *            IN - the datalength byte index within the RX packet that this TX packet will
	 *            generate.
	...
	private RS232Packet sendPacket(RS232Packet txPacket, int rxDataLenIndex) throws Exception
	{
		...
		synchronized (this.semaphore_)
		{
			...
			try
			{
				...
				{
					Thread.sleep(100); /*
										 * Wait for just a bit longer. giveing the stale bytes time
										 * to complete
										 */
				...
				/*
				 * Read the bytes on the port until we get to the start of the return packet. That
				 * means we need to first skip the tx packet
				 */
...
//			System.out.println("\n" + p.getName() + rxPacket.toString());
		...
		for (int index = 0; index < params.size(); index++)
		{
			...
			if (p.getAddress().length != SSM_OBD2_ADDRESS_SIZE)
			{
				throw new RuntimeException("This SSM Protocol allows for ONLY "
						+ SSM_OBD2_ADDRESS_SIZE + " address bytes.  Parameter " + p.getName()
						+ " has " + data.size());
			}
	 ...
	 * @return
	 **********************************************************************************************/
	private void setIdleSpeed(boolean withAirCon) throws Exception
	{
